...
{
	region_mins[0] = region_mins[1] = region_mins[2] = BOGUS_RANGE;
	region_maxs[0] = region_maxs[1] = region_maxs[2] = -BOGUS_RANGE;
}
...
qboolean CanJoinFaces (face_t *f, face_t *f2)
{
	...
	if (f2->planenum != f->planenum || f2->planeside != f->planeside || f2->texturenum != f->texturenum)
		return false;
	if (f2->outputnumber != -1)
		return false;
	if (f2->contents[0] != f->contents[0])
	{
		// does this ever happen? they shouldn't share.
	...
	if (!(texinfo[f->texturenum].flags & TEX_SPECIAL))
	{
		...
		for (i = 0;i < 3;i++)
		{
			if (region_maxs[i] - region_mins[i] > subdivide_size)
			{
